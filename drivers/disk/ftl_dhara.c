...
int dhara_nand_is_bad(const struct dhara_nand *n, dhara_block_t b)
{
	...
	ret = flash_ex_op(ctx->info.dev, NAND_FLASH_IS_BAD_BLOCK, (uintptr_t)&b, &is_bad);
	if (ret == -ENOTSUP) {
		LOG_INF("Checking bad block is not supported");
		return 0;
	} else if (ret != 0) {
		LOG_ERR("Checking bad block failed with error %d", ret);
		return 1;
	}
...
void dhara_nand_mark_bad(const struct dhara_nand *n, dhara_block_t b)
{
	...
	ret = flash_ex_op(ctx->info.dev, NAND_FLASH_MARK_BAD_BLOCK, (uintptr_t)&b, NULL);
	if (ret == -ENOTSUP) {
...
int dhara_nand_erase(const struct dhara_nand *n, dhara_block_t b, dhara_error_t *err)
{
	...
	if (ret != 0) {
		LOG_ERR("flash_erase failed with error %d", ret);
		    ...
		    dhara_error_t *err)
{
	...
	if (ret != 0) {
		LOG_ERR("flash_write failed with error %d", ret);
		*err = DHARA_E_BAD_BLOCK;
		return -1;
	}
...
int dhara_nand_is_free(const struct dhara_nand *n, dhara_page_t p)
{
	...
	if (ret != 0) {
		LOG_ERR("flash_read failed with error %d", ret);
		return 0;
	}

	for (size_t i = 0; i < ctx->page_size; i++) {
		if (ctx->page_buffer[i] != flash_params->erase_value) {
			return 0;
		}
	}
...
int disk_ftl_access_init(struct disk_info *disk)
{
	...
	ret = flash_area_open(ctx->partition_id, &area);
	if (ret != 0) {
		LOG_ERR("Opening flash area %u failed with error %d", ctx->partition_id, ret);
		return ret;
	}

	ctx->info.dev = flash_area_get_device(area);
				...
				uint32_t num_sector)
{
	...
	if ((start_sector + num_sector) > (ctx->partition_size / ctx->page_size)) {
...
#define DISK_FTL_INIT(n)                                                                           \
	...
	static struct disk_ftl_data disk_ftl_data_##n = {                                          \
		...
		.partition_id = DT_FIXED_PARTITION_ID(PARTITION_PHANDLE(n)),                       \
		.partition_offset = DT_REG_ADDR(PARTITION_PHANDLE(n)),                             \
		.partition_size = DT_REG_SIZE(PARTITION_PHANDLE(n)),                               \
